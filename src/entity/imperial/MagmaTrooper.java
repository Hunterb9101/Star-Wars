...
public class MagmaTrooper extends Entity {
	...
	Animation compileImg = new Animation(new Image[]{Registry.loadImage("stormtrooper/MagmaTrooperCompile.png")}, new int[]{0});
	
	public MagmaTrooper() {
		super("MagmaTrooper");
		buttonImg = Registry.loadImage("button/buttonMagma.png");
		...
		hitPoints = 125;
		attack = 25;
		if(Main.planet == Main.PlanetTypes.MAGMA){
			hitPoints *= 1.1;
			attack *= 1.25;
		}
	...
	public void spawn(int iX,int iY, Player p){
		super.spawn(iX, iY, p);
		appliedAddons = new Addon[]{Addon.findByName("Pauldron_Black"),Addon.findByName("Backpack")};
	}
}
